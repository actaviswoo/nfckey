#!/bin/bash
# Скрипт развертывания для nftkey.russianode.ru

set -e

DOMAIN="nftkey.russianode.ru"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_NAME="nfcshop"
SERVICE_NAME="nfcshop"
USER="www-data"

echo "🚀 Развертывание NFC Shop на $DOMAIN"
echo "===================================="
...
DEBUG=False
ALLOWED_HOSTS=nftkey.russianode.ru,138.124.74.169,localhost,127.0.0.1
...
EOF
    ...
    if ! grep -q "nftkey.russianode.ru" .env; then
        sed -i 's/ALLOWED_HOSTS=.*/ALLOWED_HOSTS=nftkey.russianode.ru,138.124.74.169,localhost,127.0.0.1/' .env || true
    fi
fi
...
[Unit]
Description=NFC Shop Gunicorn daemon
